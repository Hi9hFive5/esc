...
public interface StudyclubLogRepository extends JpaRepository<StudyclubLog, Integer> {
    ...
    @Query(value = "SELECT new org.highfives.esc.studylog.dto.StudyclubLogInfoDTO(s2.id, s1.content,s1.contentInfo, s1.studyclubId) FROM StudyclubLog s1 JOIN Studyclub s2 ON s2.id = s1.studyclubId WHERE s1.studyclubId = :studyclubId")
